...
void
ObVectorQueryVidIterator::reset() {
    is_init_ = false;
    total_ = 0;
    cur_pos_ = 0;
}
...
int
ObPluginVectorIndexHelper::merge_delta_and_snap_vids(const ObVsagQueryResult& first,
...
int
ObPluginVectorIndexHelper::get_vector_memory_value_and_limit(const uint64_t tenant_id,
                                                             int64_t& value,
                                                             int64_t& upper_limit) {
    int ret = OB_SUCCESS;
    ...
    if (tenant_config.is_valid() && OB_SUCC(ret)) {
        ...
        LOG_TRACE("check is_ob_vector_memory_valid",
                  K(value),
                  K(extra_mem_percent),
...
int
ObPluginVectorIndexHelper::is_ob_vector_memory_valid(const uint64_t tenant_id, bool& is_valid) {
    int ret = OB_SUCCESS;
    is_valid = false;
    int64_t value = 0;
...
int
ObPluginVectorIndexHelper::get_vector_memory_limit_size(const uint64_t tenant_id,
    ...
    if (OB_TMP_FAIL(GCTX.omt_->get_tenant_unit(tenant_id, unit))) {
        ...
        int64_t upper_limit = 0;
        if (OB_FAIL(get_vector_memory_value_and_limit(
                tenant_id, ob_vector_memory_limit_percentage, upper_limit))) {
            LOG_WARN("fail to get vector memory value and limit", K(ret));
        } else if (0 < ob_vector_memory_limit_percentage &&
                   ob_vector_memory_limit_percentage < upper_limit) {
            memory_limit = memory_size * ob_vector_memory_limit_percentage / 100;
            LOG_TRACE("vector index memory limit debug",
                      K(memory_size),
                      K(ob_vector_memory_limit_percentage),
                      K(memory_limit));
        } else {
            memory_limit = 0;
            LOG_TRACE("vector index memory is not enough,check memstore config",
                      K(memory_size),
                      K(ob_vector_memory_limit_percentage),
                      K(memory_limit));
        }
...
int
ObPluginVectorIndexHelper::vsag_errcode_2ob(int vsag_errcode) {
