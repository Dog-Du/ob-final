...
OB_DEF_SERIALIZE_SIZE(ObVectorIndexHNSWParam) {
    ...
    int ret = OB_SUCCESS;
    LST_DO_CODE(
        OB_UNIS_ADD_LEN, type_, lib_, dist_algorithm_, dim_, m_, ef_construction_, ef_search_);
    return len;
}
...
OB_DEF_SERIALIZE(ObVectorIndexHNSWParam) {
    int ret = OB_SUCCESS;
    LST_DO_CODE(
        OB_UNIS_ENCODE, type_, lib_, dist_algorithm_, dim_, m_, ef_construction_, ef_search_);
    return ret;
}
...
OB_DEF_DESERIALIZE(ObVectorIndexHNSWParam) {
    int ret = OB_SUCCESS;
    LST_DO_CODE(
        OB_UNIS_DECODE, type_, lib_, dist_algorithm_, dim_, m_, ef_construction_, ef_search_);
    return ret;
}
...
int
ObVectorQueryAdaptorResultContext::init_bitmaps() {
    INIT_SUCC(ret);
...
int
ObVectorQueryAdaptorResultContext::is_bitmaps_valid() {
    INIT_SUCC(ret);
...
// &str)
int
ObVectorQueryAdaptorResultContext::set_vector(int64_t index,
                                              const char* ptr,
                                              common::ObString::obstr_size_t size) {
    INIT_SUCC(ret);
    ...
    } else if (OB_ISNULL(copy_str =
                             static_cast<char*>(batch_allocator_->alloc(sizeof(char*) * size)))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
...
void
free_memdata_resource(ObVectorIndexRecordType type,
...
static int
try_free_memdata_resource(ObVectorIndexRecordType type,
                          ObVectorIndexMemData*& memdata,
                          ObIAllocator* allocator,
                          uint64_t tenant_id) {
    int ret = OB_SUCCESS;
...
ObPluginVectorIndexAdaptor::ObPluginVectorIndexAdaptor(common::ObIAllocator* allocator,
                                                       ...
                                                       uint64_t tenant_id)
    : create_type_(CreateTypeMax),
      type_(VIAT_MAX),
      algo_data_(nullptr),
      incr_data_(nullptr),
      snap_data_(nullptr),
      vbitmap_data_(nullptr),
      ...
      idle_cnt_(0),
      mem_check_cnt_(0),
      is_mem_limited_(false),
      all_vsag_use_mem_(nullptr),
      allocator_(allocator),
      parent_mem_ctx_(entity),
      index_identity_(),
...
ObPluginVectorIndexAdaptor::~ObPluginVectorIndexAdaptor() {
    int ret = OB_SUCCESS;
    LOG_INFO("destruct adaptor and free resources",
             K(is_complete()),
             K(this),
             KPC(this),
             K(lbt()));  // remove later
...
int
ObPluginVectorIndexAdaptor::init_mem(ObVectorIndexMemData*& table_info) {
    INIT_SUCC(ret);
    ...
    if (OB_NOT_NULL(table_info)) {
        ...
        LOG_WARN("failed to create vbitmap msg", K(ret));
                             ...
                             OB_NEWx(ObVsagMemContext, get_allocator(), all_vsag_use_mem_))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
...
bool
ObPluginVectorIndexAdaptor::is_mem_data_init_atomic(ObVectorIndexRecordType type) {
    bool bret = false;
    if (type == VIRT_INC) {
...
int
ObPluginVectorIndexAdaptor::init(lib::MemoryContext& parent_mem_ctx, uint64_t* all_vsag_use_mem) {
    INIT_SUCC(ret);
...
int
ObPluginVectorIndexAdaptor::init(ObString init_str,
                                 int64_t dim,
...
int
ObPluginVectorIndexAdaptor::set_param(ObString init_str, int64_t dim) {
    INIT_SUCC(ret);
...
int
ObPluginVectorIndexAdaptor::param_deserialize(char* ptr,
                                              int32_t length,
                                              ObIAllocator* allocator,
                                              ObVectorIndexAlgorithmType& type,
                                              void*& param) {
...
int
ObPluginVectorIndexAdaptor::get_dim(int64_t& dim) {
    INIT_SUCC(ret);
...
int
ObPluginVectorIndexAdaptor::get_hnsw_param(ObVectorIndexHNSWParam*& param) {
    INIT_SUCC(ret);
    if (type_ == VIAT_HNSW) {
        if (OB_ISNULL(param = static_cast<ObVectorIndexHNSWParam*>(algo_data_))) {
            ret = OB_ERR_UNEXPECTED;
...
int
ObPluginVectorIndexAdaptor::fill_vector_index_info(ObVectorIndexInfo& info) {
    int ret = OB_SUCCESS;
    // table_id
    ...
    if (OB_FAIL(databuff_printf(
            info.statistics_, sizeof(info.statistics_), pos, "is_complete=%d;", is_complete()))) {
    ...
    if (FAILEDx(databuff_printf(info.statistics_,
        ...
        LOG_WARN("failed to fill statistics", K(ret), K(this));
                   ...
                   info.statistics_, sizeof(info.statistics_), pos, "idle_cnt=%ld;", idle_cnt_))) {
        LOG_WARN("failed to fill statistics", K(ret), K(this));
    } else if (!index_identity_.empty() && OB_FAIL(databuff_printf(info.statistics_,
                                                                   sizeof(info.statistics_),
                                                                   pos,
                                                                   "index=%s;",
        ...
        LOG_WARN("failed to fill statistic", K(ret), K(this));
               ...
               OB_FAIL(databuff_printf(info.statistics_,
                                       sizeof(info.statistics_),
                                       pos,
                                       "incr_data.scn=%lu;",
    ...
    pos = 0;
    if (OB_FAIL(ret)) {
    } else if (OB_FAIL(databuff_printf(info.sync_info_,
                                       sizeof(info.sync_info_),
                                       pos,
                                       "incr_cnt=%lu;",
...
int
ObPluginVectorIndexAdaptor::init_mem_data(ObVectorIndexRecordType type) {
    INIT_SUCC(ret);
    ObVectorIndexHNSWParam* param = nullptr;
    ...
    if (OB_FAIL(get_hnsw_param(param))) {
        ...
        if (!incr_data_->is_inited()) {
            if (OB_FAIL(
                    incr_data_->mem_ctx_->init(parent_mem_ctx_, all_vsag_use_mem_, tenant_id_))) {
            ...
            if (OB_FAIL(ret)) {
                ...
                LOG_INFO("create incr index success.",
                         K(ret),
                         KP(incr_data_->index_),
                         K(lbt()));  // remove later
            }
        ...
        if (!snap_data_->is_inited()) {
            if (OB_FAIL(
                    snap_data_->mem_ctx_->init(parent_mem_ctx_, all_vsag_use_mem_, tenant_id_))) {
                LOG_WARN("failed to init incr data mem ctx.", K(ret));
            } else {
                lib::ObMallocHookAttrGuard malloc_guard(
                    lib::ObMemAttr(tenant_id_, "VIndexVsagADP"));
                if (OB_FAIL(obvectorutil::create_index(snap_data_->index_,
                    ...
                    ret = ObPluginVectorIndexHelper::vsag_errcode_2ob(ret);
                    LOG_WARN(
                        "failed to create vsag index.", K(ret), K(snap_data_->index_), KPC(param));
                }
            }

            if (OB_SUCC(ret)) {
                snap_data_->set_inited();
                LOG_INFO("create snap data success.",
                         K(ret),
                         KP(snap_data_->index_),
                         K(lbt()));  // remove later
            }
            if (OB_FAIL(ret)) {
...
void*
ObPluginVectorIndexAdaptor::get_incr_index() {
    void* res = nullptr;
    if (OB_NOT_NULL(incr_data_)) {
...
void*
ObPluginVectorIndexAdaptor::get_snap_index() {
    void* res = nullptr;
...
int
ObPluginVectorIndexAdaptor::check_tablet_valid(ObVectorIndexRecordType type) {
    INIT_SUCC(ret);
    if (type == VIRT_INC) {
...
int
ObPluginVectorIndexAdaptor::insert_rows(blocksstable::ObDatumRow* rows,
    ...
    if (OB_ISNULL(rows)) {
        ...
        } else if (OB_ISNULL(del_vids = static_cast<uint64_t*>(
                                 tmp_allocator.alloc(sizeof(uint64_t) * row_count)))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            LOG_WARN("failed to alloc del vids.", K(ret));
        } else if (OB_ISNULL(null_vids = static_cast<uint64_t*>(
                                 tmp_allocator.alloc(sizeof(uint64_t) * row_count)))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            LOG_WARN("failed to alloc del vids.", K(ret));
        } else if (OB_ISNULL(vectors = static_cast<float*>(
                                 tmp_allocator.alloc(sizeof(float) * row_count * dim)))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
        ...
        for (int i = 0; OB_SUCC(ret) && i < row_count; i++) {
            ...
            // }
            else if (OB_ISNULL(vector = (float*)(vector_datum.ptr().ptr_))) {
                ret = OB_ERR_UNEXPECTED;
        ...
        if (OB_SUCC(ret)) {
            ...
            if (OB_FAIL(obvectorutil::add_index(incr_data_->index_,
                ...
                incr_data_->curr_vid_max_ =
                    incr_data_->curr_vid_max_ > max_vid ? incr_data_->curr_vid_max_ : max_vid;
            }
        }
        if (OB_SUCC(ret)) {
            lib::ObMallocHookAttrGuard malloc_guard(lib::ObMemAttr(tenant_id_, "VIBitmapADP"));
            ...
            ROARING_TRY_CATCH(
                roaring::api::roaring64_bitmap_add_many(incr_data_->bitmap_->insert_bitmap_,
                                                        incr_vid_count,
...
int
ObPluginVectorIndexAdaptor::add_snap_index(
    float* vectors, int64_t* vids, int num, char* datas, uint32_t row_length) {
    INIT_SUCC(ret);
    int64_t dim = 0;
    if (OB_FAIL(check_tablet_valid(VIRT_SNAP))) {
        LOG_WARN("check tablet id invalid.", K(ret));
        ...
        lib::ObMallocHookAttrGuard malloc_guard(lib::ObMemAttr(tenant_id_, "VIndexVsagADP"));
        TCWLockGuard lock_guard(snap_data_->mem_data_rwlock_);
        if (OB_FAIL(obvectorutil::add_index(
                snap_data_->index_, vectors, vids, dim, num, datas, row_length))) {
            ret = ObPluginVectorIndexHelper::vsag_errcode_2ob(ret);
            LOG_WARN("failed to build index.", K(ret), K(dim), K(num));
        }
    }
...
int
ObPluginVectorIndexAdaptor::check_delta_buffer_table_readnext_status(
    ObVectorQueryAdaptorResultContext* ctx, common::ObNewRowIterator* row_iter, SCN query_scn) {
    INIT_SUCC(ret);
    SCN min_delta_scn;
...
int
ObPluginVectorIndexAdaptor::write_into_delta_mem(ObVectorQueryAdaptorResultContext* ctx,
                                                 int count,
                                                 float* vectors,
                                                 uint64_t* vids) {
    INIT_SUCC(ret);
    if (count == 0) {
        ...
        if (check_if_complete_delta(ctx->bitmaps_->insert_bitmap_, count)) {
            ...
            lib::ObMallocHookAttrGuard malloc_guard(lib::ObMemAttr(tenant_id_, "VIndexVsagADP"));
            if (OB_SUCC(ret) &&
                OB_FAIL(obvectorutil::add_index(incr_data_->index_,
                                                vectors,
                                                reinterpret_cast<int64_t*>(vids),
                                                ctx->get_dim(),
                                                count,
                                                ctx->get_param_data()->row_data_,
                                                ctx->get_param_data()->row_length_))) {
                ret = ObPluginVectorIndexHelper::vsag_errcode_2ob(ret);
...
int
ObPluginVectorIndexAdaptor::complete_delta_buffer_table_data(
    ...
    if (OB_ISNULL(ctx) || OB_ISNULL(ctx->batch_allocator_)) {
    ...
    } else if (OB_ISNULL(vids = static_cast<uint64_t*>(
                             tmp_allocator.alloc(sizeof(uint64_t) * ctx->get_vec_cnt())))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
        LOG_WARN("failed to alloc new mem.", K(ret));
    } else {
        ...
        for (int i = 0; OB_SUCC(ret) && i < ctx_vec_cnt; i++) {
            ...
            if (ctx->vec_data_.vectors_[i].is_null() ||
                ...
                ret = OB_ERR_UNEXPECTED;
                LOG_WARN("get invalid string.",
                         K(ret),
                         K(i),
                         K(ctx->vec_data_.vectors_[i].get_string().length()),
                         K(dim));
            } else if (OB_ISNULL(vector = reinterpret_cast<float*>(
...
int
ObPluginVectorIndexAdaptor::check_index_id_table_readnext_status(
    ObVectorQueryAdaptorResultContext* ctx, common::ObNewRowIterator* row_iter, SCN query_scn) {
    INIT_SUCC(ret);
    blocksstable::ObDatumRow* datum_row = nullptr;
    int64_t read_num = 0;
    ...
    if (OB_FAIL(ret)) {
        ...
        LOG_WARN(
            "failed to check comple index mem data.", K(ret), K(read_scn), K(vbitmap_data_->scn_));
...
int
ObPluginVectorIndexAdaptor::check_snapshot_table_wait_status(
    ObVectorQueryAdaptorResultContext* ctx) {
    INIT_SUCC(ret);
...
int
ObPluginVectorIndexAdaptor::write_into_index_mem(int64_t dim,
                                                 SCN read_scn,
                                                 ObArray<uint64_t>& i_vids,
    ...
    if (read_scn > vbitmap_data_->scn_) {
        ...
        LOG_TRACE(
            "write into index mem.", K(ret), K(i_vids.count()), K(d_vids.count()), K(read_scn));
    }
...
bool
ObPluginVectorIndexAdaptor::check_if_complete_index(SCN read_scn) {
    bool res = false;
...
int
ObPluginVectorIndexAdaptor::add_datum_row_into_array(blocksstable::ObDatumRow* datum_row,
                                                     ObArray<uint64_t>& i_vids,
                                                     ObArray<uint64_t>& d_vids) {
    INIT_SUCC(ret);
    int64_t vid = 0;
...
int
ObPluginVectorIndexAdaptor::complete_index_mem_data(SCN read_scn,
...
bool
ObPluginVectorIndexAdaptor::check_if_complete_delta(roaring::api::roaring64_bitmap_t* gene_bitmap,
                                                    int64_t count) {
    bool res = false;
    ...
    if (gene_vid_cnt == 0 && count > 0) {
        ...
        if (!roaring64_bitmap_is_subset(gene_bitmap, delta_bitmap)) {
            res = true;
                   ...
                   is_mem_data_init_atomic(VIRT_BITMAP)) {  // andnot_bitmap is null, if
                                                            // count = 0, do nothing
...
int
ObPluginVectorIndexAdaptor::prepare_delta_mem_data(roaring::api::roaring64_bitmap_t* gene_bitmap,
                                                   ObArray<uint64_t>& i_vids,
                                                   ObVectorQueryAdaptorResultContext* ctx) {
    INIT_SUCC(ret);
    ...
    if (OB_FAIL(try_init_mem_data(VIRT_INC))) {
        LOG_WARN("failed to init mem data incr.", K(ret));
               ...
               OB_ISNULL(delta_bitmap = incr_data_->bitmap_->insert_bitmap_) || OB_ISNULL(ctx) ||
        ...
        if (OB_FAIL(ret)) {
            ...
            if (OB_FAIL(ret)) {
                ...
                LOG_WARN("failed to get dim.", K(ret));
            } else if (OB_ISNULL(vids = static_cast<ObObj*>(
                                     ctx->tmp_allocator_->alloc(sizeof(ObObj) * bitmap_cnt)))) {
                ret = OB_ALLOCATE_MEMORY_FAILED;
                LOG_WARN("failed to allocator.", K(ret), K(bitmap_cnt));
            } else if (OB_ISNULL(ctx->vec_data_.vectors_ = static_cast<ObObj*>(
                                     ctx->tmp_allocator_->alloc(sizeof(ObObj) * vector_cnt)))) {
                ret = OB_ALLOCATE_MEMORY_FAILED;
                LOG_WARN("failed to allocator.", K(ret), K(bitmap_cnt));
            } else {
            ...
            if (OB_FAIL(ret)) {
                ...
                ret = OB_ERR_UNEXPECTED;
                LOG_WARN("get invalid vid iter count.",
                         K(ret),
                         K(index),
                         K(roaring64_bitmap_get_cardinality(andnot_bitmap)));
            } else {
                for (int64_t i = 0; OB_SUCC(ret) && i < i_vids.count() && i + index < bitmap_cnt;
                     i++) {
                    vids[i + index].reset();
...
int
ObPluginVectorIndexAdaptor::serialize(ObIAllocator* allocator,
...
int
ObPluginVectorIndexAdaptor::generate_snapshot_valid_bitmap(ObVectorQueryAdaptorResultContext* ctx,
                                                           common::ObNewRowIterator* row_iter,
                                                           SCN query_scn) {
    INIT_SUCC(ret);
...
int
ObPluginVectorIndexAdaptor::merge_and_generate_bitmap(ObVectorQueryAdaptorResultContext* ctx,
    ...
    if (OB_ISNULL(ctx)) {
        ...
        ROARING_TRY_CATCH(ibitmap =
                              roaring64_bitmap_flip_closed(insert_map, insert_min, curr_vid_max));
        if (OB_SUCC(ret)) {
...
// for debug version
int
ObPluginVectorIndexAdaptor::print_bitmap(roaring::api::roaring64_bitmap_t* bitmap) {
    INIT_SUCC(ret);
    if (OB_NOT_NULL(bitmap)) {
        ...
        if (bitmap_cnt == 0) {
            // do nothing
        } else if (OB_ISNULL(nums = static_cast<uint64_t*>(
...
void
ObPluginVectorIndexAdaptor::print_vids(uint64_t* vids, int64_t count) {
    if (count != 0) {
        ObArenaAllocator tmp_allocator("VectorAdaptor", OB_MALLOC_NORMAL_BLOCK_SIZE, tenant_id_);
        ObStringBuffer buffer(&tmp_allocator);
...
void
ObPluginVectorIndexAdaptor::print_vectors(float* vecs, int64_t count, int64_t dim) {
    if (count != 0) {
        ObArenaAllocator tmp_allocator("VectorAdaptor", OB_MALLOC_NORMAL_BLOCK_SIZE, tenant_id_);
        // ObStringBuffer buffer(&tmp_allocator);
...
int
ObPluginVectorIndexAdaptor::vsag_query_vids(ObVectorQueryAdaptorResultContext* ctx,
...
// for dubug
    ...
    if (OB_FAIL(ret)) {
        ...
        const ObVsagQueryResult delta_data = {
            delta_res_cnt, delta_vids, delta_distances, delta_row_datas, delta_row_length};
        const ObVsagQueryResult snap_data = {
            snap_res_cnt, snap_vids, snap_distances, snap_row_datas, snap_row_length};

        if (delta_data.row_length_ != 0 && snap_data.row_length_ != 0) {
            assert(delta_data.row_length_ == snap_data.row_length_);
        }

        merge_row_length = delta_data.row_length_ < snap_data.row_length_ ? snap_data.row_length_
                                                                          : delta_data.row_length_;

        uint64_t tmp_result_cnt = delta_res_cnt + snap_res_cnt;
        uint64_t max_res_cnt =
            tmp_result_cnt < query_cond->query_limit_ ? tmp_result_cnt : query_cond->query_limit_;
        ...
        if (max_res_cnt == 0) {
            ...
            // res_vid_array
            actual_res_cnt = 0;
        } else if (OB_ISNULL(merge_vids = static_cast<int64_t*>(
        ...
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(vids_iter->init(actual_res_cnt,
                                           merge_vids,
                                           ctx->allocator_,
                                           merge_row_datas,
                                           merge_row_length))) {
            LOG_WARN(
                "iter init failed.", K(ret), K(actual_res_cnt), K(merge_vids), K(ctx->allocator_));
    ...
    if (delta_res_cnt != 0) {
        ...
        if (delta_row_datas != nullptr) {
            incr_data_->mem_ctx_->Deallocate((void*)delta_row_datas);
            delta_row_datas = nullptr;
        }
    }
    ...
    if (snap_res_cnt != 0) {
        ...
        if (snap_row_datas != nullptr) {
            snap_data_->mem_ctx_->Deallocate((void*)snap_row_datas);
            snap_row_datas = nullptr;
        }
    }
...
int
ObPluginVectorIndexAdaptor::query_result(ObVectorQueryAdaptorResultContext* ctx,
                                         ObVectorQueryConditions* query_cond,
                                         ObVectorQueryVidIterator*& vids_iter) {
    INIT_SUCC(ret);
    vids_iter = nullptr;
    ...
    if (OB_ISNULL(ctx) || OB_ISNULL(query_cond)) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN(
            "get vector objct unexpect.", K(ret), K(query_cond->query_vector_.length()), K(dim));
    } else if (OB_ISNULL(query_vector =
                             reinterpret_cast<float*>(query_cond->query_vector_.ptr()))) {
        ret = OB_ERR_UNEXPECTED;
        ...
        if (OB_SUCC(ret) &&
            OB_FAIL(vsag_query_vids(ctx, query_cond, dim, query_vector, vids_iter))) {
            LOG_WARN("failed to query vids.", K(ret), K(dim));
        }
    }
...
int
ObPluginVectorIndexAdaptor::cast_roaringbitmap_to_stdmap(
    const roaring::api::roaring64_bitmap_t* bitmap,
    ...
    } else if (OB_ISNULL(buf = static_cast<uint64_t*>(
                             tmp_allocator.alloc(sizeof(uint64_t) * bitmap_cnt)))) {
        ret = OB_ALLOCATE_MEMORY_FAILED;
...
int
ObPluginVectorIndexAdaptor::set_tablet_id(ObVectorIndexRecordType type, ObTabletID tablet_id) {
    int ret = OB_SUCCESS;
...
int
ObPluginVectorIndexAdaptor::set_table_id(ObVectorIndexRecordType type, uint64_t table_id) {
    int ret = OB_SUCCESS;
...
int
ObPluginVectorIndexAdaptor::set_index_identity(ObString& index_identity) {
    int ret = OB_SUCCESS;
...
void
ObPluginVectorIndexAdaptor::set_vid_rowkey_info(ObVectorIndexSharedTableInfo& info) {
...
bool
ObPluginVectorIndexAdaptor::is_complete() {
    return is_inc_tablet_valid() && is_vbitmap_tablet_valid() && is_snap_tablet_valid() &&
           is_data_tablet_valid() && (vbitmap_table_id_ != OB_INVALID_ID) &&
           (inc_table_id_ != OB_INVALID_ID) && (snapshot_table_id_ != OB_INVALID_ID);
}

static int
ref_memdata(ObVectorIndexMemData*& dst_mem_data, ObVectorIndexMemData*& src_mem_data) {
    int ret = OB_SUCCESS;
...
int
ObPluginVectorIndexAdaptor::merge_mem_data_(ObVectorIndexRecordType type,
    ...
    if (OB_ISNULL(src_adapter) || OB_ISNULL(src_mem_data)) {
        ...
        ret = OB_ERR_UNEXPECTED;
        LOG_WARN("conflict use of src_mem_data",
                 K(type),
                 KPC(src_mem_data),
                 KPC(dst_mem_data),
                 K(lbt()));
    ...
    if (OB_FAIL(ret) || is_same_mem_data) {
        ...
        if (OB_NOT_NULL(dst_mem_data) &&
            OB_FAIL(try_free_memdata_resource(type, dst_mem_data, allocator_, tenant_id_))) {
...
int
ObPluginVectorIndexAdaptor::merge_parital_index_adapter(
    ...
    if (OB_ISNULL(partial_idx_adpt)) {
        ...
        if (partial_idx_adpt->is_inc_tablet_valid()) {
            if (OB_FAIL(set_tablet_id(VIRT_INC, partial_idx_adpt->get_inc_tablet_id()))) {
                LOG_WARN("partial vector index adapter not valid",
                         K(partial_idx_adpt),
                         K(*this),
                         KR(ret));
                ...
                LOG_WARN("partial vector index adapter not valid",
                         ...
                         K(*this),
                         KR(ret));
            } else if (OB_FAIL(merge_mem_data_(
                           VIRT_INC, partial_idx_adpt, partial_idx_adpt->incr_data_, incr_data_))) {
                LOG_WARN("partial vector index adapter not valid",
                         K(partial_idx_adpt),
                         K(*this),
                         KR(ret));
            ...
            if (OB_FAIL(set_tablet_id(VIRT_BITMAP, partial_idx_adpt->get_vbitmap_tablet_id()))) {
                LOG_WARN("partial vector index adapter not valid",
                         ...
                         KR(ret));
                           ...
                           set_table_id(VIRT_BITMAP, partial_idx_adpt->get_vbitmap_table_id()))) {
                LOG_WARN("partial vector index adapter not valid",
                         K(partial_idx_adpt),
                         K(*this),
                         KR(ret));
                ...
                LOG_WARN("partial vector index adapter not valid",
                         ...
                         K(*this),
                         KR(ret));
            } else if (OB_FAIL(merge_mem_data_(VIRT_BITMAP,
                                               partial_idx_adpt,
            ...
            if (OB_FAIL(set_tablet_id(VIRT_SNAP, partial_idx_adpt->get_snap_tablet_id()))) {
                LOG_WARN("partial vector index adapter not valid",
                         ...
                         KR(ret));
                           ...
                           set_table_id(VIRT_SNAP, partial_idx_adpt->get_snapshot_table_id()))) {
                LOG_WARN("partial vector index adapter not valid",
                         K(partial_idx_adpt),
                         K(*this),
                         KR(ret));
                ...
                LOG_WARN("partial vector index adapter not valid",
                         ...
                         K(*this),
                         KR(ret));
            } else if (OB_FAIL(merge_mem_data_(VIRT_SNAP,
                                               partial_idx_adpt,
        ...
        if (OB_SUCC(ret) && OB_ISNULL(algo_data_) && OB_NOT_NULL(partial_idx_adpt->algo_data_)) {
...
void
ObPluginVectorIndexAdaptor::inc_ref() {
...
bool
ObPluginVectorIndexAdaptor::dec_ref_and_check_release() {
...
int
ObPluginVectorIndexAdaptor::check_need_sync_to_follower(bool& need_sync) {
    int ret = OB_SUCCESS;
    need_sync = false;
    ...
    if (!is_complete()) {
        ...
        if (current_incr_count >
                follower_sync_statistics_.incr_count_ + VEC_INDEX_INCR_DATA_SYNC_THRESHOLD ||
            current_bitmap_count >
                follower_sync_statistics_.vbitmap_count_ + VEC_INDEX_INCR_DATA_SYNC_THRESHOLD ||
            current_snapshot_count !=
                follower_sync_statistics_.snap_count_) {  // use scn_ in memdata for compare
            need_sync = true;
            LOG_INFO("need sync to follower",
                     K(follower_sync_statistics_),
                     K(current_incr_count),
                     K(current_bitmap_count),
                     K(current_snapshot_count),
                     KPC(this));
        } else {
...
// debug function
void
ObPluginVectorIndexAdaptor::output_bitmap(roaring::api::roaring64_bitmap_t* bitmap) {
...
int
ObPluginVectorIndexAdaptor::check_vsag_mem_used() {
    INIT_SUCC(ret);
    int64_t mem_size = 0;
    ...
    if (is_mem_limited_ || mem_check_cnt_ % 10 == 0) {
        ...
        if (OB_FAIL(
                ObPluginVectorIndexHelper::get_vector_memory_limit_size(tenant_id_, mem_size))) {
            ...
            LOG_WARN("Memory usage exceeds user limit.",
                     K(ret),
                     K(mem_size),
                     K(ATOMIC_LOAD(all_vsag_use_mem_)),
                     K(bitmap_mem_used));
        }
    }
...
int
ObPluginVectorIndexAdaptor::get_incr_vsag_mem_used() {
    int64_t size = 0;
    if (incr_data_->is_inited()) {
...
int
ObPluginVectorIndexAdaptor::get_incr_vsag_mem_hold() {
    int64_t size = 0;
    if (incr_data_->is_inited()) {
...
int
ObPluginVectorIndexAdaptor::get_snap_vsag_mem_used() {
    int64_t size = 0;
    if (snap_data_->is_inited()) {
...
int
ObPluginVectorIndexAdaptor::get_snap_vsag_mem_hold() {
    int64_t size = 0;
    if (snap_data_->is_inited()) {
...
void*
ObVsagMemContext::Allocate(size_t size) {
    if (size == 0) {
        return nullptr;
    } else {
...
void
ObVsagMemContext::Deallocate(void* p) {
...
void*
ObVsagMemContext::Reallocate(void* p, size_t size) {
    if (size == 0) {
        if (p != nullptr) {
            Deallocate(p);
        }
...
int
ObVsagMemContext::init(lib::MemoryContext& parent_mem_context,
                       uint64_t* all_vsag_use_mem,
                       uint64_t tenant_id) {
    INIT_SUCC(ret);
