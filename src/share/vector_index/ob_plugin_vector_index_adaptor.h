...
public:
  ...
  TO_STRING_KV(K_(ls_id), K_(rowkey_vid_table_id), K_(vid_rowkey_table_id),
               K_(inc_index_table_id), K_(vbitmap_table_id),
               K_(snapshot_index_table_id), K_(data_table_id),
               K_(rowkey_vid_tablet_id), K_(vid_rowkey_tablet_id),
               K_(inc_index_tablet_id), K_(vbitmap_tablet_id),
               K_(snapshot_index_tablet_id), K_(data_tablet_id), K_(statistics),
               K_(sync_info));
...
enum ObVectorIndexAlgorithmLib { VIAL_VSAG = 0, VIAL_MAX };

enum ObVectorIndexAlgorithmType { VIAT_HNSW = 0, VIAT_MAX };
...
struct ObVectorIndexHNSWParam {
  ObVectorIndexHNSWParam()
      : type_(VIAT_MAX), lib_(VIAL_MAX), dim_(0), m_(0), ef_construction_(0),
        ef_search_(0) {}
  void reset() {
    type_ = VIAT_MAX;
    lib_ = VIAL_MAX;
...
public:
  TO_STRING_KV(K_(type), K_(lib), K_(dist_algorithm), K_(dim), K_(m),
               K_(ef_construction), K_(ef_search));
};
...
public:
  ...
  ObVectorQueryAdaptorResultContext(uint64_t tenant_id, ObIAllocator *allocator,
                                    ObIAllocator *tmp_allocator,
                                    ObIAllocator *batch_allocator)
      : status_(PVQ_START), flag_(PVQP_MAX), tenant_id_(tenant_id),
        bitmaps_(nullptr), vec_data_(), allocator_(allocator),
        tmp_allocator_(tmp_allocator), batch_allocator_(batch_allocator) {};
  ...
  int64_t get_vec_cnt() {
    return vec_data_.count_ - vec_data_.curr_idx_ >
  ...
  int set_vector(int64_t index, const char *ptr,
...
private:
  ...
  ObIAllocator *allocator_; // allocator for vec_lookup_op, used to allocate
                            // memory for final query result
  ObIAllocator
      *tmp_allocator_; // used to temporarily allocate memory during the query
                       // process and does not affect the final query results
  ObIAllocator
      *batch_allocator_; // Used to complete_delta_buffer_data in batches, reuse
                         // after each batch of data is completed
};
...
struct ObVectorIndexMemData {
  ObVectorIndexMemData()
      : is_init_(false), rb_flag_(true), mem_data_rwlock_(), bitmap_rwlock_(),
        scn_(), ref_cnt_(0), curr_vid_max_(0), index_(nullptr),
        bitmap_(nullptr), mem_ctx_(nullptr) {}

public:
  TO_STRING_KV(K(rb_flag_), K_(is_init), K_(scn), K_(ref_cnt), K_(curr_vid_max),
               KP_(index), KPC_(bitmap), KP_(mem_ctx));
  void free_resource(ObIAllocator *allocator_);
  bool is_inited() const { return is_init_; }
  void set_inited() { is_init_ = true; }
  void inc_ref() {
    ATOMIC_INC(&ref_cnt_);
    // OB_LOG(INFO, "inc ref count", K(ref_cnt_), KP(this), KPC(this),
    // K(lbt())); // remove later
  }
  bool dec_ref_and_check_release() {
    int64_t ref_count = ATOMIC_SAF(&ref_cnt_, 1);
    // OB_LOG(INFO,"dec ref count", K(ref_count), KP(this), KPC(this),
    // K(lbt())); // remove later
    return (ref_count == 0);
  }
...
public:
  ObVectorIndexFollowerSyncStatic()
      : incr_count_(0), vbitmap_count_(0), snap_count_(0), sync_count_(0),
        sync_fail_(0), idle_count_(0) {}
  void reset() {
    incr_count_ = 0;
    vbitmap_count_ = 0;
    snap_count_ = 0;
...
struct ObVectorIndexSharedTableInfo {
  ObVectorIndexSharedTableInfo()
      : rowkey_vid_table_id_(OB_INVALID_ID),
        vid_rowkey_table_id_(OB_INVALID_ID), data_table_id_(OB_INVALID_ID),
        rowkey_vid_tablet_id_(), vid_rowkey_tablet_id_() {}
  bool is_valid() {
  ...
  TO_STRING_KV(K_(rowkey_vid_table_id), K_(vid_rowkey_table_id),
               K_(rowkey_vid_tablet_id), K_(vid_rowkey_tablet_id),
               K_(data_table_id));
...
public:
  ...
  ObPluginVectorIndexAdaptor(common::ObIAllocator *allocator,
                             lib::MemoryContext &entity, uint64_t tenant_id);
  ~ObPluginVectorIndexAdaptor();

  int init(ObString init_str, int64_t dim, lib::MemoryContext &parent_mem_ctx,
           uint64_t *all_vsag_use_mem);
  // only used for background maintance handle aux table no.4 / 5 before get
  // index aux table no.3
  ...
  // VSAG ADD
  int insert_rows(blocksstable::ObDatumRow *rows, const int64_t vid_idx,
  ...
  static int
  cast_roaringbitmap_to_stdmap(const roaring::api::roaring64_bitmap_t *bitmap,
                               std::map<int, bool> &mymap, uint64_t tenant_id);
  int check_vsag_mem_used();
  uint64_t get_all_vsag_mem_used() { return ATOMIC_LOAD(all_vsag_use_mem_); }
  ...
  int complete_index_mem_data(SCN read_scn, common::ObNewRowIterator *row_iter,
  ...
  int prepare_delta_mem_data(roaring::api::roaring64_bitmap_t *gene_bitmap,
                             ObArray<uint64_t> &i_vids,
                             ObVectorQueryAdaptorResultContext *ctx);
  int serialize(ObIAllocator *allocator, ObOStreamBuf::CbParam &cb_param,
  ...
  int64_t get_sync_idle_count() {
    return follower_sync_statistics_.idle_count_;
  }
  ...
  TO_STRING_KV(K_(create_type), K_(type), KP_(algo_data), KP_(incr_data),
               KP_(snap_data), KP_(vbitmap_data), K_(tenant_id),
               K_(data_tablet_id), K_(rowkey_vid_tablet_id),
               K_(vid_rowkey_tablet_id), K_(inc_tablet_id),
               K_(vbitmap_tablet_id), K_(snapshot_tablet_id), K_(data_table_id),
               K_(rowkey_vid_table_id), K_(vid_rowkey_table_id),
               K_(inc_table_id), K_(vbitmap_table_id), K_(snapshot_table_id),
               K_(ref_cnt), K_(idle_cnt), KP_(allocator), K_(index_identity),
               K_(follower_sync_statistics), K_(mem_check_cnt),
               K_(is_mem_limited));
...
private:
  ...
  bool check_if_complete_delta(roaring::api::roaring64_bitmap_t *gene_bitmap,
                               int64_t count);
  int write_into_delta_mem(ObVectorQueryAdaptorResultContext *ctx, int count,
                           float *vectors, uint64_t *vids);
  int write_into_index_mem(int64_t dim, SCN read_scn, ObArray<uint64_t> &i_vids,
                           ObArray<uint64_t> &d_vids);
  ...
  int vsag_query_vids(ObVectorQueryAdaptorResultContext *ctx,
                      ObVectorQueryConditions *query_cond, int64_t dim,
                      float *query_vector,
                      ObVectorQueryVidIterator *&vids_iter);
...
private:
  ...
  ObString index_identity_; // identify multi indexes on one table & column,
                            // generate unique uint64 to save memory?
  ...
  constexpr const static char
      *const VEC_INDEX_ALGTH[ObVectorIndexDistAlgorithm::VIDA_MAX] = {
          "l2",
          "ip",
          "cos",
  };
...
public:
  ...
  int set_adapter(ObPluginVectorIndexAdaptor *adapter) {
    ...
    if (is_valid()) {
      ...
      OB_LOG(WARN, "vector index adapter guard can only set once",
             KPC(adapter_), KPC(adapter));
    } else {
      adapter_ = adapter;
...
public:
  ObVsagMemContext(uint64_t *all_vsag_use_mem)
      : all_vsag_use_mem_(all_vsag_use_mem), mem_context_(nullptr) {};
  ~ObVsagMemContext() {
  ...
  int64_t hold() { return mem_context_->hold(); }
