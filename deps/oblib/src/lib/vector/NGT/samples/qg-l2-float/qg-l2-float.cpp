...
main(int argc, char** argv) {
    ...
    size_t maxNumberOfEdges = 96;
    try {
        std::cout << "quantizing the index... : " << dimensionOfSubvector << "  "
                  << maxNumberOfEdges << std::endl;
        NGTQG::Index::quantize(indexPath, dimensionOfSubvector, maxNumberOfEdges, false);
    } catch (NGT::Exception& err) {
        cerr << "Error " << err.what() << endl;
        return 1;
    } catch (...) {
