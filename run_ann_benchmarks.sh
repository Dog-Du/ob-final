...
## 测试时导入数据并构建索引
time python run.py --algorithm oceanbase --local --force --dataset sift-128-euclidean --runs 7 --skip_fit
## 测试时跳过导入数据及构建索引
...
# 重启 oceanbase 集群
~/oceanbase-2024/oceanbase/tools/deploy/obd.sh start -n obcluster cluster restart obcluster
